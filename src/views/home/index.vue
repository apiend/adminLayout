          ...
          <tr v-for="item in tableData">
            ...
            <td>
              <el-button
                type="success"
                size="mini"
                @click="goLink('/monitor',{name:item.proId})"
              >监控任务
              </el-button>
              <el-button
                type="info"
              ...
              <el-button
                type="warning"
                size="mini"
                @click="doPop(item.proId,0)"
                v-if="item.proOperStatus == 1 "
              >关闭
              </el-button>
              <el-button
                type="danger"
                size="mini"
                @click="doPop(item.proId,1)"
                v-else
              >开启</el-button>
              <el-button
                type="danger"
                size="mini"
                @click="delWarning(item.proId)"
                v-if="item.proOperStatus == 0 "
              >删除
              </el-button>
            </td>
          </tr>
    ...
    >
      <el-row>
        ...
        <el-col class="mt10">
          ...
          <el-col :span="16">
            <el-input
              placeholder="请输入该项目的服务器访问地址http://xxxx.mdb "
              v-model.trim="$v.projectHost.$model"
            ></el-input>
            <div
              class="error"
              v-if="!$v.projectHost.maxLength"
            >长度不能超过100位</div>
          </el-col>
        </el-col>
      </el-row>
  ...
  return Vue.component("v-home", {
    ...
    methods: {
      ...
      doPop: function(id, status) {
        ...
        let child = document.getElementsByClassName("special-redTips")[0];
        if (parent && child) {
        ...
        if (status == 0) {
          setTimeout(() => {
            ...
            tmp.innerHTML = "*该项目当前运行的监控任务都会停止";
            tmp.classList.add("special-redTips");
            document
        ...
        let msgTip =
          status == 0 ? "确定要改为 关闭 吗？" : "确定要改为 开启 吗？";
        that
          .$confirm(msgTip, "操作确认", {
